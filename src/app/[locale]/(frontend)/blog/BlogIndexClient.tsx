...
const CATEGORIES_EN = ['View all', 'Product Guides', 'Growth research', 'Muscle studies', 'Recovery protocols', 'Metabolic research']
const CATEGORIES_ES = ['Ver todo', 'Guías de Producto', 'Investigación de crecimiento', 'Estudios musculares', 'Protocolos de recuperación', 'Investigación metabólica']
const MOCK_IMAGES = [
...
export default function BlogIndexPage({ payloadPosts = [] }: { payloadPosts?: BlogListItem[] }) {
  const locale = useLocale()
  const t = useTranslations('blog')
  const CATEGORIES = locale === 'es' ? CATEGORIES_ES : CATEGORIES_EN
  const BLOG_POSTS = useMemo(
    () => [...payloadPosts, ...(locale === 'es' ? BLOG_POSTS_ES : BLOG_POSTS_EN)],
    [locale, payloadPosts],
  )
        ...
        <FadeUp delay={0.4}>
          ...
          <Link href={`/${latestPost?.slug}`} className="group flex flex-col lg:flex-row gap-6 lg:gap-12 items-stretch w-full bg-white p-4 sm:p-6 lg:p-8 rounded-[2rem] lg:rounded-[2.5rem] border border-ink/5 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
            ...
            <div className="w-full lg:w-[45%] flex flex-col justify-center px-2 sm:px-4 lg:px-6 py-4 lg:py-8 min-w-0">
              ...
              <h2 className="text-3xl sm:text-4xl lg:text-3xl xl:text-4xl font-extrabold font-sans text-ink mb-4 sm:mb-6 tracking-tight leading-[1.1] group-hover:text-primary transition-colors duration-500 line-clamp-3">
                {latestPost?.title}
              </h2>
