  ...
  return (
    <main className="bg-cream min-h-screen pt-32 pb-24">
      ...
      <section className="px-6 mb-16 max-w-page mx-auto">
        <FadeUp>
          <EyebrowHeading gold>{t('eyebrow')}</EyebrowHeading>
          <h1 className="text-display-lg font-serif text-ink mt-4 mb-6">{t('title')}</h1>
      ...
      <section className="px-6 mb-32 max-w-page mx-auto">
        <FadeUp delay={0.1}>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <h2 className="text-editorial-md font-serif text-ink">{t('libraryTitle')}</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border-strong">
                  <th className="py-4 px-4 text-label-md uppercase tracking-wider text-ink-muted font-normal">{t('tableProduct')}</th>
                  <th className="py-4 px-4 text-label-md uppercase tracking-wider text-ink-muted font-normal">{t('tablePurity')}</th>
                  <th className="py-4 px-4 text-label-md uppercase tracking-wider text-ink-muted font-normal">{t('tableBatch')}</th>
                  <th className="py-4 px-4 text-label-md uppercase tracking-wider text-ink-muted font-normal">{t('tableAnalyzed')}</th>
                  <th className="py-4 px-4 text-label-md uppercase tracking-wider text-ink-muted font-normal text-right">{t('tableCoaDownload')}</th>
                </tr>
              </thead>
              <tbody>
              ...
              <div key={coa.id} className="bg-cream-warm border border-border-subtle rounded-md p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-label-sm uppercase tracking-wider text-ink-muted">{t('purityLabel')}</span>
                  </div>
                </div>

                <div className="flex justify-between items-center py-3 border-t border-b border-border-subtle mb-4">
                  <div>
                    <span className="block text-label-sm uppercase tracking-wider text-ink-muted mb-1">{t('batchLabel')}</span>
                    <span className="text-body-sm font-mono text-ink">{coa.batch}</span>
                  </div>
                  <div className="text-right">
                    <span className="block text-label-sm uppercase tracking-wider text-ink-muted mb-1">{t('analyzedLabel')}</span>
      ...
      <section className="px-6 mb-32 bg-cream-sand py-24">
        <div className="max-w-page mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <FadeUp delay={0.1}>
              <EyebrowHeading gold>{t('processEyebrow')}</EyebrowHeading>
              <h2 className="text-display-sm font-serif text-ink mt-4 mb-8">{t('processTitle')}</h2>
      ...
      <section className="px-6 text-center max-w-content mx-auto">
        <FadeUp>
          <h2 className="text-editorial-lg font-serif text-ink mb-6">{t('ctaTitle')}</h2>
          <p className="text-body-lg text-ink-muted mb-10">
            {t('ctaText')}
          </p>
          <Link href="/science">
            <Button variant="primary" size="lg" className="gap-2">
              {t('ctaButton')} <ArrowRightIcon className="w-4 h-4" />
            </Button>
          </Link>
        </FadeUp>
      </section>
