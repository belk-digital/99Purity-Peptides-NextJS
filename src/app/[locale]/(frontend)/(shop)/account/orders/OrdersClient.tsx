        ...
        <div className="flex items-center gap-3 bg-white p-1 rounded-2xl shadow-sm border border-gray-100">
          ...
          <Select value={filter} onValueChange={setFilter}>
            ...
            <SelectContent className="bg-white border-gray-100 rounded-xl shadow-xl shadow-black/5">
              <SelectItem value="all" className="text-[11px] font-bold uppercase tracking-[0.1em] rounded-lg">{t('filterAll')}</SelectItem>
              <SelectItem value="processing" className="text-[11px] font-bold uppercase tracking-[0.1em] rounded-lg">{t('filterProcessing')}</SelectItem>
              <SelectItem value="delivered" className="text-[11px] font-bold uppercase tracking-[0.1em] rounded-lg">{t('filterDelivered')}</SelectItem>
              <SelectItem value="returned" className="text-[11px] font-bold uppercase tracking-[0.1em] rounded-lg text-red-500">{t('filterReturned')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
              ...
              >
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-12 flex-1">
                  ...
                  <div className="flex flex-col gap-2 min-w-[140px]">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">{t('orderPlaced')}</span>
                  ...
                  {/* Status Badge */}
                  <div className="flex flex-col gap-2">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 hidden md:block">{t('status')}</span>
                  ...
                  <div className="flex flex-col gap-2 mt-4 md:mt-0 md:ml-auto">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 hidden md:block text-right">{t('total')}</span>
                    <div className="flex items-end gap-2 md:justify-end">
                      <span className={`text-2xl font-bold text-black tracking-tight ${spaceGrotesk.className}`}>
                        ${order.total.toFixed(2)}
                      </span>
                      <span className="text-xs font-medium text-gray-400 mb-1">{t('itemCount', { count: order.itemCount })}</span>
                    </div>
                  </div>
