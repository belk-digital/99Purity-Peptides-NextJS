...
export default function CheckoutSummary({ subtotal, shippingZones, processingFees }: CheckoutSummaryProps) {
  const t = useTranslations('checkout.checkoutSummary')
  // Coupon State
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/5 border border-white/10 rounded-lg p-6">
        <h2 className="text-xl font-medium mb-4">{t('shippingMethod')}</h2>
        {defaultZone ? (
          <div className="space-y-4">
            <p className="text-sm text-gray-400 mb-2">{t('zoneLabel', { name: defaultZone.name })}</p>
              ...
              <div key={idx} className="flex items-center justify-between border border-white/10 p-4 rounded-md">
                <div className="flex items-center">
                  ...
                  <label htmlFor={`shipping-${idx}`} className="ml-3 block text-sm font-medium text-white">
                    {m.method}
                    {m.estimatedDays && <span className="text-gray-400 ml-2">{t('estimatedDays', { days: m.estimatedDays })}</span>}
                  </label>
                </div>
          ...
          <p className="text-sm text-red-400">{t('noShippingZones')}</p>
        )}
      </div>
      ...
      {processingFees.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-lg p-6">
          <h2 className="text-xl font-medium mb-4">{t('additionalOptions')}</h2>
          <div className="space-y-4">
            {processingFees.map((fee) => {
              const feeValue = fee.type === 'percentage'
                ? t('feePercentageOfSubtotal', { amount: fee.amount })
                ...
                <div key={fee.id} className="flex items-center">
                  ...
                  {fee.isOptional ? (
                    ...
                    <div className="ml-3 text-sm font-medium text-white flex-1 flex justify-between">
                      <span>{t('feeRequired', { name: fee.name })}</span>
                      <span className="text-gray-400">{feeValue}</span>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-lg p-6">
        ...
        <div className="space-y-2 text-sm text-gray-300">
          ...
          <div className="flex justify-between">
            <span>{t('shippingWithMethod', { method: selectedMethod })}</span>
            <span>
              {freeShipping ? (
                <span className="text-green-400">{t('free')}</span>
              ) : (
