...
export function AgeGate() {
  ...
  const springDragX = useSpring(dragX, { damping: 25, stiffness: 200 })
  
  // Transform the thumb's horizontal drag into vertical modal movement
  // Drag Right = Move Down. Drag Left = Stay Still (prevent lifting off the bottom edge)
  const modalOffset = useTransform(springDragX, [-120, 0, 120], [0, 0, 150])
      ...
      {isVisible && (
        <div className="fixed inset-0 z-[999999] pointer-events-auto">
          ...
          >
            ...
            <motion.div
              style={{ y: modalOffset }}
              className={`absolute bottom-0 left-0 right-0 bg-cream shadow-[0_-20px_60px_rgba(0,0,0,0.15)] flex flex-col md:flex-row overflow-hidden pointer-events-auto border-t border-white/50 transition-[height,border-radius,opacity] duration-700 ease-in-out ${isExpanding ? 'h-[100vh] rounded-none opacity-0' : 'h-[85vh] md:h-[60vh] rounded-t-[2rem] md:rounded-t-[3rem] opacity-100'}`}
            >
              {/* Text Content Pane */}
              <div className="relative w-full md:w-[55%] lg:w-[60%] p-6 sm:p-10 md:p-16 flex flex-col bg-transparent shrink-0 flex-1 md:h-full overflow-y-auto order-last md:order-first">
                
                <div className="max-w-xl w-full flex flex-col items-center sm:items-start text-center sm:text-left pb-12 sm:pb-0 m-auto">
                  <p className="font-bold tracking-[0.2em] uppercase text-primary text-[10px] md:text-xs mb-3">
                    Restricted Access
                  </p>
                  <h2 className="text-4xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-ink mb-6 tracking-tighter font-heading uppercase leading-none">
                    Age<br className="hidden sm:block" /> Verification
                  </h2>
                  
                  <div className="w-12 h-[3px] bg-primary mb-6 mx-auto sm:mx-0" />

                  <div className="text-ink/80 text-sm md:text-base leading-relaxed mb-10 space-y-4 font-medium max-w-md">
                    <p>
                      <strong className="text-ink block mb-1 text-base md:text-lg">RESEARCH PURPOSES ONLY</strong>
                  ...
                  <div className="relative w-full max-w-[340px] h-[72px] bg-black/[0.04] border border-black/5 rounded-full shadow-inner flex items-center justify-between px-6 mx-auto sm:mx-0 mt-2">
                    ...
                    <motion.div 
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="absolute top-1/2 left-1/2 mt-[-28px] ml-[-28px] w-14 h-14 bg-black text-white rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.2)] flex items-center justify-center cursor-grab active:cursor-grabbing z-10 transition-shadow"
                    >
                ...
                <div className="mt-12 text-ink/30 text-[9px] uppercase tracking-[0.2em] text-center sm:text-left">
                  By entering, you agree to our Terms of Service
                </div>
              </div>

              {/* Image Pane */}
              <div className="relative w-full md:w-[45%] lg:w-[40%] h-[25vh] sm:h-[30vh] md:h-full bg-black shrink-0 order-first md:order-last">
                <Image 
